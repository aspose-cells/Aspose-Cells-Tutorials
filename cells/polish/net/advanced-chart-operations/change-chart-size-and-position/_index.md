...
Teraz, gdy mamy załadowany skoroszyt, musimy uzyskać dostęp do konkretnego arkusza, z którym chcemy pracować, w tym przypadku jest to pierwszy arkusz (indeks `[0]`). Podobnie jak przewracanie się na właściwą stronę w książce, ten krok pomaga nam skupić się na arkuszu, który chcemy edytować.
...
Teraz, gdy mamy odpowiedni rozmiar, przesuńmy wykres! Zmieniając `X` I `Y` właściwości, zasadniczo zmieniamy położenie wykresu na arkuszu. Wyobraź sobie, że przeciągasz oprawione zdjęcie w nowe miejsce na ścianie, aby lepiej pokazać jego piękno!
