...
## Importuj pakiety
Zanim zaczniemy kodować, upewnij się, że dodałeś niezbędne przestrzenie nazw. Importowanie odpowiednich pakietów pozwoli Ci bezproblemowo pracować z funkcjami Aspose.Cells.
```csharp
using System.IO;
using Aspose.Cells;
```
...
Tutaj dzieje się akcja! Zaczniemy od ukrycia trzeciego wiersza w arkuszu kalkulacyjnym.
```csharp
...
### Czy istnieje sposób na pokazanie ukrytych wierszy i kolumn?  
 Oczywiście! Możesz użyć`UnhideRow()` I`UnhideColumn()` metody ponownego wyświetlenia ukrytych wierszy i kolumn.
...
### Czy mogę zastosować tę metodę do wielu arkuszy w jednym skoroszycie?  
 Tak, poprzez pętlę`Worksheets` skoroszycie można stosować akcje ukrywania i pokazywania do wielu arkuszy.
