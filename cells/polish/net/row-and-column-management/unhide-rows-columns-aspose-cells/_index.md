...
```
 W powyższym kodzie,`2` odnosi się do indeksu wiersza (pamiętaj, że jest on liczony od zera) i`13.5` ustawia wysokość tego wiersza. Dostosuj te wartości według potrzeb w konkretnym przypadku.
...
```
 Ponownie,`1` jest indeksem kolumny zaczynającym się od zera, a`8.5` określa szerokość tej kolumny. Modyfikuj te parametry w zależności od swoich wymagań.
...
fstream.Close();
```
I to wszystko! Udało Ci się odkryć wiersze i kolumny w pliku Excela za pomocą Aspose.Cells dla .NET.
## Wniosek
