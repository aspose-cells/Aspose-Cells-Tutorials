...
### Importuj bibliotekę
Teraz, gdy pakiet został już zainstalowany, musisz uwzględnić go w kodzie.
```csharp
using System.IO;
using Aspose.Cells;
```
...
```
 W tym wierszu mówisz Aspose.Cells, aby pobrał Twój skoroszyt (dzieło sztuki) i zapisał go jako plik XML o nazwie`output.xml` używając formatu SpreadsheetML.`SaveFormat.SpreadsheetML` w ten sposób Aspose wie, w jakim formacie zapisać plik.
## Wniosek
...
Niezależnie od tego, czy tworzysz aplikacje biznesowe, panele raportowania czy cokolwiek pomiędzy, opanowanie języka Aspose.Cells niewątpliwie wzbogaci Twój zestaw narzędzi do kodowania o cenne narzędzie.
