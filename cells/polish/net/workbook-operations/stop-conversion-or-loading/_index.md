...
using System.IO;
```
Podzielmy proces na szczegółowe kroki. Każdy krok pomoże Ci zrozumieć znaczenie konfiguracji i używania Interrupt Monitor do zarządzania konwersją skoroszytu programu Excel.
...
## Krok 4: Zapisz skoroszyt jako PDF i obsłuż przerwanie
 Teraz spróbujmy zapisać skoroszyt jako PDF. Użyjemy`try-catch` blok umożliwiający obsługę ewentualnych przerw.
```csharp
try
{
...
```
To opóźnienie daje skoroszytowi czas na rozpoczęcie konwersji do formatu PDF przed wysłaniem sygnału przerwania.
