...
```
 Zastępować`"Your Document Directory"` rzeczywistą ścieżką, w której przechowywany jest plik Excela i gdzie chcesz zapisać podpisany plik.
...
Aby dodać podpis cyfrowy, musisz załadować plik PFX i podać dla niego hasło. Oto, jak możesz to zrobić:
```csharp
string password = "pfxPassword"; // Zastąp swoim hasłem PFX
string pfx = "pfxFile"; // Ścieżka do pliku PFX
```
 Pamiętaj o wymianie`"pfxPassword"` z Twoim prawdziwym hasłem i`"pfxFile"` ze ścieżką do pliku PFX.
## Krok 4: Utwórz podpis cyfrowy
 Teraz czas na utworzenie podpisu cyfrowego za pomocą`DigitalSignature` Klasa. Będziesz musiał odczytać plik PFX do tablicy bajtów, a następnie utworzyć podpis.
```csharp
...
### Czy potrzebuję certyfikatu cyfrowego, aby używać podpisów XAdES?
