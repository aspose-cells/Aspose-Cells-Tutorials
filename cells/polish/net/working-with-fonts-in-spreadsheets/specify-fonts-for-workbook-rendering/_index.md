...
```
To tak, jakby pakować walizki na wycieczkę. `LoadOptions` służą jako niezbędne rzeczy w podróży – przygotowują skoroszyt do nadchodzącej podróży (proces renderowania). Łącząc `fontConfigs` Do `opts`zapewniasz, że po załadowaniu skoroszytu zostanie on rozpoznany i będzie szukał Twoich niestandardowych czcionek.
