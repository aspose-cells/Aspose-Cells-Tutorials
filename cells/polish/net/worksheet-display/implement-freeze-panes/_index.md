...
Następnie musimy załadować plik Excel, aby Aspose.Cells mogło działać swoją magią. Aby to zrobić, utworzymy strumień plików i otworzymy plik Excel za pomocą tego strumienia.
```csharp
...
```
 Myśleć`Workbook` jako segregator, który trzyma wszystkie twoje arkusze razem. Po otwarciu segregatora możesz uzyskać dostęp do dowolnej strony (arkusza roboczego) w nim zawartej.
...
## Krok 5: Zastosuj ustawienia zamrożenia paneli
 Tutaj dzieje się magia! Aby ustawić szyby zamrażarki, użyj`FreezePanes`metodę, określając wiersz i kolumnę, w których ma nastąpić zamrożenie, a także liczbę wierszy i kolumn do zamrożenia.
```csharp
