...
Dodajmy kolejną wartość, powiedzmy do komórki `A3`sformatuj go tak, aby wyświetlał się jako waluta. Aby było ciekawiej, użyjemy formatu, który wyświetla wartości dodatnie jako walutę w funtach, a wartości ujemne jako dolary.
```csharp
...
- Ustawiliśmy niestandardowy format `£#,##0;[Red]$-#,##0`, który wyświetla wartości dodatnie ze znakiem funta, a wartości ujemne na czerwono ze znakiem dolara.
...
Oczywiście! Możesz użyć niestandardowych formatów liczbowych, aby wyświetlać liczby ujemne za pomocą różnych kolorów lub symboli.
