...
category: general
date: 2026-06-21
description: Docker imajı nasıl oluşturulur ve doğru port eşlemesiyle Docker konteyneri
  nasıl çalıştırılır öğrenin. Docker run port eşlemesi ve Docker’da port expose etmeyi
  içerir.
draft: false
keywords:
- build docker image
...
language: tr
og_description: Doğru port eşlemesiyle Docker imajı oluşturun ve Docker konteynerini
  çalıştırın. Docker run port eşlemesini ustaca yönetin ve dakikalar içinde Docker’da
  portu açın.
og_title: Docker Görüntüsü Oluşturma ve Docker Konteynerini Çalıştırma – Tam Kılavuz
schemas:
- author: Aspose
...
- containers
- devops
title: Docker Görüntüsü Oluşturma ve Docker Konteynerini Çalıştırma – Tam Kılavuz
url: /tr/python/import-and-export/build-docker-image-and-run-docker-container-complete-guide/
---
...
# Docker Görüntüsü Oluşturma ve Docker Konteyneri Çalıştırma – Tam Kılavuz

Hiç **build docker image** bir basit web uygulaması için nasıl yapılır ve ardından sorunsuz bir şekilde nasıl çalıştırılır diye merak ettiniz mi? Yalnız değilsiniz—birçok geliştirici konteynerle çalışmaya ilk adım attıklarında aynı duvara çarpar. Bu öğreticide, bir Dockerfile yazmaktan doğru portu açmaya ve son olarak `docker run` ile bu portu ana makinenize eşlemeye kadar tüm süreci adım adım inceleyeceğiz. Sonunda **run docker container** nasıl doğru port eşlemesiyle yapılır tam olarak bilecek ve Docker’da bir portun açılmasının neden önemli olduğunu göreceksiniz.

İhtiyacınız olan her şeyi kapsayacağız: tam `docker build` komutu, **docker build from Dockerfile**, `docker run port mapping` incelikleri ve konteynerin gerçekten dinlediğini kontrol etmek için hızlı bir doğrulama. Fazla laf yok, sadece terminalinize kopyalayıp yapıştırabileceğiniz, adım adım bir kılavuz.

## What You'll Achieve

- Node.js (veya herhangi bir) uygulama için minimal bir Dockerfile yazın.  
- Resmi CLI sözdizimini kullanarak **build docker image** oluşturun.  
- Dockerfile’daki `EXPOSE` ile `docker run`’daki `-p` bayrağı arasındaki farkı anlayın.  
- **run docker container** `docker run port mapping` ile `http://localhost:5000` adresinden hizmete ulaşın.  
- Unutulan portlar veya eşleşmeyen host‑container portları gibi yaygın tuzakları teşhis edin.

### Prerequisites

- Docker Engine kurulu (Desktop ya da Engine 20.10+).  
- Komut satırıyla temel aşinalık.  
- Küçük bir web uygulaması (tek satırlık bir Python Flask sunucusu kullanacağız, ama istediğiniz başka bir şeyle de değiştirebilirsiniz).  
