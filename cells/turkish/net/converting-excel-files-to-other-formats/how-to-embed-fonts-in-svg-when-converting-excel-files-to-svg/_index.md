...
category: general
date: 2026-09-15
description: SVG'ye font eklemeyi ve Excel grafiğini PowerPoint'e dışa aktarmayı öğrenin;
  XLSX'i SVG'ye ve XLSX'i PPTX'e dönüştürmeyi tam kod örnekleriyle kapsayan.
draft: false
images:
...
language: tr
lastmod: 2026-09-15
og_description: SVG'ye yazı tiplerini göm ve Excel grafiğini adım adım C# kodu ile
  PowerPoint'e aktar. XLSX'i hızlı ve güvenilir bir şekilde SVG'ye ve PPTX'e dönüştür.
og_image_alt: Screenshot showing an Excel chart exported to PowerPoint with embedded
  fonts in the resulting SVG file
og_title: SVG'ye Yazı Tipi Gömme ve Excel Grafiğini PowerPoint'e Aktarma – Tam Kılavuz
schemas:
- author: Aspose
...
- C#
- Excel automation
title: Excel dosyalarını SVG ve PowerPoint'e dönüştürürken SVG'ye fontları nasıl gömülür?
url: /tr/net/converting-excel-files-to-other-formats/how-to-embed-fonts-in-svg-when-converting-excel-files-to-svg/
---
...
# Excel dosyalarını SVG ve PowerPoint'e Dönüştürürken SVG'ye Yazı Tipi Gömme  
...
Excel çalışma kitabını SVG'ye dönüştürürken **SVG'ye yazı tiplerini gömmeniz** gerekiyorsa, bu kılavuz tam olarak nasıl yapılacağını gösterir. Ayrıca **Excel grafiğini PowerPoint'e dışa aktarmayı**, **XLSX'i SVG'ye dönüştürmeyi** ve **XLSX'i PPTX'e dönüştürmeyi** düzenlenebilir grafiklerle nasıl yapacağınızı öğreneceksiniz.  
...
Excel verileriyle programlı olarak çalışmak, genellikle aynı görsel içeriği farklı dosya formatları arasında taşımanız gerektiği anlamına gelir. PowerPoint'te bir grafiği manuel olarak yeniden oluşturmak veya bir SVG'de yazı tiplerini yeniden uygulamak hataya açık ve zaman alıcıdır. Bu öğreticinin sonunda, tek bir yeniden kullanılabilir C# kod parçacığına sahip olacaksınız:
...
* Çalışma kitabını gömülü yazı tipleri ve yazı tipi varyasyon seçicileriyle bir SVG dosyası olarak kaydeder.  
* Aynı çalışma kitabını grafiğin düzenlenebilir kaldığı bir PPTX dosyasına dışa aktarır.  
...
Tek gereksinim, **Aspose.Cells for .NET**'in (2024‑x veya daha yeni) güncel bir sürümü ve Visual Studio 2022 gibi bir .NET geliştirme ortamıdır.
...
* En az bir grafik içeren bir Excel dosyası (`input.xlsx`).  
* Çıktı dizinine yazma izni.  

---

## XLSX'i SVG'ye Dönüştürürken SVG'ye Yazı Tipi Gömme  
...
Yazı tiplerini gömmek, SVG'nin hedef sistemde orijinal yazı tipleri bulunmasa bile herhangi bir cihazda doğru şekilde görüntülenmesini sağlar. `SvgSaveOptions` sınıfı, bunu mümkün kılan iki bayrak sunar: `EmbedFonts` ve `FontVariationSelectors`.
...
* `EmbedFonts = true` yazı tipi dosyalarını SVG'nin `<defs>` bölümüne kopyalar, dış bağımlılıkları ortadan kaldırır.  
...
* `FontVariationSelectors = true` OpenType özelliklerini destekleyen yazı tipleri için gerekli seçicileri ekler, ligaturalar gibi glif varyasyonlarını korur.  
...
**Beklenen sonuç:** `WithFonts.svg` dosyasını herhangi bir modern tarayıcıda açın; grafik veya hücre içindeki metin, Excel'de kullanılan tam yazı tipiyle, o yazı tipi yüklü olmayan makinelerde bile görünür.  

---

## Excel Grafiğini Düzenlenebilir Grafiklerle PowerPoint'e Dışa Aktarma  
...
Bir grafiği PowerPoint slaytına gömmeniz ve alıcının grafik verilerini düzenlemesine izin vermeniz gerektiğinde, Aspose.Cells'in `PptxSaveOptions` sınıfı `ExportEditableChart` bayrağını sunar.
...
`ExportEditableChart` değerini `true` olarak ayarlamak, grafiği statik bir görüntü yerine Office Open XML grafik nesnesi olarak depolar. `EditableChart.pptx` dosyasını PowerPoint'te açtığınızda, grafiğe sağ‑tıklayıp → **Edit Data** seçeneğiyle serileri, yerel bir PowerPoint grafiği gibi değiştirebilirsiniz.
...
1. `EditableChart.pptx` dosyasını PowerPoint'te açın.  
2. Grafiği içeren slaytı bulun.  
3. **Chart Tools → Design → Edit Data** seçeneğini seçin.  
4. Excel benzeri veri ızgarasının göründüğünden ve değerleri değiştirebildiğinizden emin olun.  

---

## XLSX'i SVG'ye Dönüştür – Tam İş Akışı Özeti  
...
Aşağıda, yükleme, isteğe bağlı veri manipülasyonu ve SVG olarak kaydetmeyi birleştiren kompakt bir sürüm bulunmaktadır. Sadece SVG çıktısına ihtiyacınız olduğunda bunu kullanın.
...
**Köşe durumları ipucu:** Çalışma kitabınız sunucuda yüklü olmayan özel yazı tipleri içeriyorsa, `Save` metodunu çağırmadan önce bunları manuel olarak gömün. `FontInfoCollection` kullanarak yazı tipi dosyalarını `SvgSaveOptions`'a `CustomFonts` özelliği üzerinden ekleyebilirsiniz (daha yeni Aspose.Cells sürümlerinde mevcuttur).  

---

## XLSX'i PPTX'e Dönüştür – Grafik Düzenlenebilirliğini Koruma  

The following helper method demonstrates the **convert XLSX to PPTX** path while ensuring the chart remains editable.

Aşağıdaki yardımcı yöntem, **XLSX'i PPTX'e dönüştür** yolunu gösterirken grafiğin düzenlenebilir kalmasını sağlar.
...
**Sık sorulan soru:** *Çalışma kitabımda birden fazla grafik içeren sayfa varsa ne olur?*  
**Cevap:** Aspose.Cells varsayılan olarak ilk sayfayı dışa aktarır. Ek sayfaları dahil etmek için `workbook.Worksheets` üzerinde döngü yapın, her grafiği yeni bir slayta kopyalayın ve her slaytı ayrı ayrı `Presentation` nesneleriyle Aspose.Slides kullanarak kaydedin. Bu gelişmiş senaryo, temel “çalışma kitabını SVG olarak kaydet” ve “Excel grafiğini PowerPoint'e dışa aktar” akışının ötesindedir, ancak temel bayraklar aynı kalır.  
...
* **Performans:** Yazı tiplerini gömmek SVG dosya boyutunu artırır. Boyut bir sorun ise `EmbedFonts = false` ayarlayın ve web‑güvenli yazı tiplerine güvenin.  

* **Font licensing:** Ensure you have the right to embed the fonts you use; some commercial fonts restrict embedding.  
* **Yazı tipi lisansı:** Kullandığınız yazı tiplerini gömmek için hakkınızın olduğundan emin olun; bazı ticari yazı tipleri gömme işlemini kısıtlar.  
...
* **Grafik uyumluluğu:** Düzenlenebilir grafikler PPTX içinde `chart.xml` parçaları olarak kaydedilir. Çok karmaşık grafikler (ör. 3‑D veya kombinasyon grafikler) PowerPoint'te düzenlenirken bazı stil öğelerini kaybedebilir. En çok ihtiyaç duyduğunuz grafik türlerini test edin.  
...
* **Sürüm uyumsuzlukları:** `ExportEditableChart` bayrağı Aspose.Cells 20.10 veya daha yeni bir sürüm gerektirir. Daha eski bir sürüm kullanmak, sessizce raster görüntüye geri döner.  

* **Thread safety:** Workbook objects are not thread‑safe. Create a new `Workbook` instance per request in a web service scenario.  
* **İş parçacığı güvenliği:** Workbook nesneleri iş parçacığı güvenli değildir. Web servisi senaryosunda her istek için yeni bir `Workbook` örneği oluşturun.  
...
Bu programı çalıştırdığınızda iki dosya üretilir:

* **WithFonts.svg** – an SVG that renders exactly like the Excel view, fonts included.  
* **WithFonts.svg** – Excel görünümüyle aynı şekilde render edilen, yazı tipleri dahil bir SVG.  

* **EditableChart.pptx** – a PowerPoint presentation where the chart can be edited directly.  
* **EditableChart.pptx** – grafiğin doğrudan düzenlenebildiği bir PowerPoint sunumu.  
...
Artık **XLSX'i SVG'ye dönüştürürken SVG'ye yazı tiplerini nasıl gömeceğinizi** ve **Excel grafiğini PowerPoint'e dışa aktarırken grafiği düzenlenebilir tutmayı** biliyorsunuz. Aynı kod, **çalışma kitabını SVG olarak kaydetme** ve **XLSX'i PPTX'e dönüştürme** işlemlerini minimum çabayla nasıl temiz bir şekilde yapabileceğinizi de gösterir.

From here you can explore further topics such as:

Buradan itibaren aşağıdaki gibi konuları keşfedebilirsiniz:

* Adding custom fonts programmatically (`svgOptions.CustomFonts`).  
* Özel yazı tiplerini programlı olarak ekleme (`svgOptions.CustomFonts`).  

* Batch‑processing multiple workbooks in a background service.  
* Arka plan servisinde birden fazla çalışma kitabını toplu işleme.  

* Using Aspose.Slides to create multi‑slide PPTX files that combine several Excel charts.  
* Birden fazla Excel grafiğini birleştiren çok slaytlı PPTX dosyaları oluşturmak için Aspose.Slides kullanma.  
...
Seçeneklerle deneyler yapın, kod parçacıklarını projenize uyarlayın ve manuel sonrası işleme gerek kalmadan güvenilir Excel‑to‑SVG/PPTX dönüşümlerinin keyfini çıkarın. Kodlamanın tadını çıkarın!
