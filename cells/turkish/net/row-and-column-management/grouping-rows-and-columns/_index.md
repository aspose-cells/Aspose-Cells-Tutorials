...
Aspose.Cells, dosyayı bir dosya akışı aracılığıyla açmanızı gerektirir. Bu akış, işleme için dosyanın içeriğini okur ve yükler.
```csharp
...
Tıpkı satırlarda olduğu gibi, daha temiz ve daha düzenli bir düzen oluşturmak için sütunları gruplayabilirsiniz. İlk üç sütunu gruplamanın yolu şöyledir.
```csharp
...
### Satırları veya sütunları gruplandırmayı kaldırmak istersem ne olur?  
 Kullanmak`worksheet.Cells.UngroupRows(startRow, endRow)` veya`worksheet.Cells.UngroupColumns(startColumn, endColumn)` onları gruplandırmak için.
