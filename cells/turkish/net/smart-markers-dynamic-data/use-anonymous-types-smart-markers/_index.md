...
Şimdi, çalışma sayfasına akıllı işaretçiler eklememiz gerekiyor. Bu işaretçiler daha sonra gireceğimiz veriler için yer tutucular olacak.
```csharp
...
Şimdi, kullanıcı adımızı kullanarak bir liste oluşturalım. `Person` Akıllı işaretçileri doldurmak için kullanacağımız sınıf.
```csharp
