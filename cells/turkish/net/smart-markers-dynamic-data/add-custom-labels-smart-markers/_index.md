...
d.SetDataSource(dt);
```
Bu adımda yeni bir tane oluşturuyoruz `WorkbookDesigner` nesne ve belirterek `designer` çalışma kitabını hedef çalışma kitabı olarak belirledik. Daha sonra akıllı işaretçiler için veri kaynağını `DataTable` bir önceki adımda oluşturduğumuz.
