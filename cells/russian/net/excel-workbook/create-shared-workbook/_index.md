...
```
Создавая экземпляр `Workbook` класс, мы готовим новую книгу в памяти. Представьте себе, что вы открываете новый чистый лист в Excel, ожидая заполнения данными.
...
wb.Параметрs.Shared = true;
```
Setting `wb.Settings.Shared` к `true` указывает, что к этой рабочей книге могут получить доступ несколько пользователей одновременно. Это как раздать друзьям ключи от дома — они могут приходить и уходить, когда им вздумается!
