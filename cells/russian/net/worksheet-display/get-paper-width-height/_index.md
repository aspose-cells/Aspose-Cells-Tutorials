...
Начнем с создания экземпляра `Workbook` класс. Это формирует основу нашей манипуляции файлами Excel.
```csharp
...
Далее мы перейдем к первому рабочему листу в нашей недавно созданной рабочей книге. Это довольно просто:
```csharp
...
```
Вуаля! В декларации будут напечатаны конкретные высота и ширина для бумаги формата А3.
