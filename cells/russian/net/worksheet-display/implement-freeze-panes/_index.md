...
## Импортные пакеты
Давайте начнем с импорта необходимых пакетов. Откройте свой проект C# и обязательно импортируйте их:
```csharp
using System.IO;
using Aspose.Cells;
```
...
```
Используя файловый поток, вы открываете файл для доступа Aspose.Cells, не изменяя исходный файл, пока вы явно не сохраните какие-либо изменения.
...
- Первый столбец (2): начать заморозку со столбца 2.
...
```
Обязательно сохраните его под другим именем, если вы хотите сохранить и исходный файл.
