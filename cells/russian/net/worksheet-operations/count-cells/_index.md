...
А теперь самое интересное! Мы собираемся написать код, который открывает файл Excel и подсчитывает количество ячеек на одном из его листов. Внимательно следуйте этим шагам:
...
Теперь мы готовы подсчитать клетки. `Cells` Коллекция рабочего листа содержит все ячейки в этом конкретном листе. Вы можете получить доступ к общему количеству ячеек следующим образом:
```csharp
