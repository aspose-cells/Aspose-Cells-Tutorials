...
Далее давайте создадим экземпляр `LoadOptions`. Вот где начинается магия. Настроив параметры загрузки, вы можете настроить обратный вызов, который будет срабатывать всякий раз, когда при загрузке рабочей книги возникает предупреждение:
```csharp
