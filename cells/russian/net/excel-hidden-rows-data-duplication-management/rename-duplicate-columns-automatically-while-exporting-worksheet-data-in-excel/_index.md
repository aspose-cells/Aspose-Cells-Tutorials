...
Теперь пришло время добавить некоторые данные, а именно настроить наши столбцы. В нашем примере столбцы A, B и C будут иметь одинаковое имя «People».
```csharp
...
```
 Мы создаем переменную`columnName` чтобы сохранить наше имя и затем присвоить его ячейкам A1, B1 и C1. Это похоже на размещение трех одинаковых этикеток на трех разных банках.
...
```
 Этот цикл проходит через столбцы`DataTable`и выводит на консоль название каждого столбца. Это удовлетворение от того, что наши банки выстроены в ряд, маркированы и готовы к использованию.
## Заключение
