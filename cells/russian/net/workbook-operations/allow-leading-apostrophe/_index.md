...
## Импортные пакеты
Как только вы все настроите и подготовите, следующим шагом будет импорт необходимых пакетов. Вот как вы можете сделать это эффективно:
...
Теперь, когда вы импортировали необходимые пакеты, давайте рассмотрим подробное пошаговое руководство по разрешению использования начальных апострофов в рабочей книге Excel.
...
workbook.Параметрs.QuotePrefixToStyle = false;
```
Setting `QuotePrefixToStyle` значение false позволяет корректно обрабатывать начальные апострофы. 
