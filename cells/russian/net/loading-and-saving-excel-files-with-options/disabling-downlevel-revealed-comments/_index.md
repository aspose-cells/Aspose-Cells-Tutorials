...
```
На этом этапе замените `"Your Document Directory"` с фактическими путями к файлам в вашей системе. Вы также можете создавать пользовательские каталоги для лучшей организации входных и выходных файлов.
...
Теперь нам нужно настроить объект HtmlSaveOptions, чтобы отключить нижние уровни раскрытых комментариев. Эта опция гарантирует, что любые комментарии или скрытый контент не будут раскрыты в конечном HTML-файле.
```csharp
// Создайте новый объект HtmlSaveOptions для настройки параметров сохранения.
HtmlSaveOptions opts = new HtmlSaveOptions();
// Отключить просмотр комментариев нижнего уровня
...
Да, просто установите `DisableDownlevelRevealedComments` собственность `false` если вы хотите включить комментарии нижнего уровня при сохранении книги в формате HTML.
