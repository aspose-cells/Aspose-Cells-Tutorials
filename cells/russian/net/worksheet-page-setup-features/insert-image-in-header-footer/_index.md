...
Во-первых, нам нужно указать путь к нашему каталогу документов, где находятся файл Excel и изображение. Вы можете указать любой путь, просто замените `"Your Document Directory"` с фактическим путем к вашему каталогу.
```csharp
...
Для работы с изображением нам необходимо прочитать его в массив байтов. Это важно, поскольку позволяет нам манипулировать изображением в приложении.
```csharp
...
Вот он, волшебный момент! Вставим наш логотип в центральную часть заголовка:
```csharp
