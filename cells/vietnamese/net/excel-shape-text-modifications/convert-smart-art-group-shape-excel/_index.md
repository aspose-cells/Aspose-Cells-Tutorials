...
Tiếp theo, chúng ta cần xác định hình dạng thực tế mà chúng ta quan tâm. Trong trường hợp này, chúng ta đang lấy hình dạng đầu tiên trên bảng tính.
```csharp
...
Tiếp theo, chúng ta muốn kiểm tra xem hình dạng đó có phải là hình dạng nhóm hay không. 
```csharp
...
```
Dòng mã này thực hiện chuyển đổi. Nếu thành công, Smart Art của bạn giờ là Group Shape!
