...
```
Trong bước này, chúng ta lấy kiểu của cột đầu tiên, thiết lập `IsLocked` ĐẾN `true`và áp dụng khóa cho cột đó bằng cách sử dụng `StyleFlag`Thao tác này bảo vệ cột đầu tiên khỏi mọi chỉnh sửa.
...
```
Trong bước này, chúng ta lưu sổ làm việc vào thư mục đã chỉ định trước đó, đặt tên cho tệp đầu ra là `output.out.xls`. Bạn có thể thay đổi tên tệp hoặc định dạng tùy theo nhu cầu.
## Phần kết luận
