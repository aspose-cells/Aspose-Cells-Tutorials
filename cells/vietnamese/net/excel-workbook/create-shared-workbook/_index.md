...
Bây giờ chúng ta sẽ đi vào phần hấp dẫn! Sau đây là cách tạo sổ làm việc chung bằng Aspose.Cells cho .NET, được chia thành các bước dễ thực hiện.
...
```
 Bằng cách khởi tạo`Workbook` lớp, chúng ta chuẩn bị một bảng tính mới trong bộ nhớ. Hãy nghĩ về điều này như việc mở một trang tính trống mới trong Excel, chờ được điền dữ liệu.
...
```
 Ở đây, chúng tôi đang lưu sổ làm việc được chia sẻ của mình với tên`outputSharedWorkbook.xlsx` trong thư mục đầu ra được chỉ định. Thao tác này sẽ tạo ra một tệp Excel mà giờ đây có thể được chia sẻ và sửa đổi bởi nhiều người dùng.
