...
```csharp
// Khởi tạo sổ làm việc mẫu có chứa dấu hiệu thông minh với ISBLANK
...
designer.Process();
```
Với cuộc gọi đơn giản này `Process()`, các điểm đánh dấu thông minh trong sổ làm việc của chúng tôi sẽ được điền dữ liệu tương ứng từ `DataSet`, bao gồm cả các đánh giá trống theo yêu cầu.
