...
category: general
date: 2026-05-30
description: 使用 C# 更改 Excel 中文本框字型大小。學習如何快速透過一步一步的程式碼修改 Excel 文本框字型。
draft: false
keywords:
- change textbox font size
- modify excel textbox font
language: zh-hant
og_description: 使用 C# 更改 Excel 文字方塊的字型大小。本指南示範如何安全且高效地修改 Excel 文字方塊的字型。
og_title: 使用 C# 更改 Excel 文字方塊字型大小 – 完整教學
schemas:
- author: Aspose
...
- C#
- Office Automation
title: 使用 C# 在 Excel 中更改文字方塊字體大小 – 完整指南
url: /zh-hant/net/excel-shape-text-modifications/change-textbox-font-size-in-excel-with-c-complete-guide/
---
...
# 在 Excel 中使用 C# 調整文字方塊字型大小 – 完整指南

需要在 C# 中 **變更 Excel 工作表的文字方塊字型大小** 嗎？您來對地方了。無論是產生報表、打造儀表板，或只是微調範本，調整文字方塊的外觀都能讓您的試算表看起來更專業。

在本教學中，我們還會 **修改 Excel 文字方塊字型**，不僅限於大小——包括字型族、粗體，甚至處理多個圖形。完成後，您將擁有一段可直接執行的程式碼，涵蓋從開啟活頁簿到釋放 COM 物件的每個步驟。沒有冗長說明，只有實用程式碼，今天就能放入您的專案。
...
|-------------|----------------|
| **.NET 6+** (or .NET Framework 4.7.2+) | 提供 C# 編譯器與執行環境。 |
| **Microsoft.Office.Interop.Excel** NuGet package | 提供與 Excel 溝通所需的 COM interop 類型。 |
| **Excel installed** (any recent version) | Interop 層只能在安裝了 Office 應用程式時運作。 |
| **Basic C# knowledge** | 您可以輕鬆跟隨，我們會逐行說明。 |

若缺少任何項目，請先暫停並安裝完成；本指南的後續內容皆假設已具備上述環境。
...
![顯示 Excel 工作表並以高亮文字方塊示範如何變更文字方塊字型大小](change-textbox-font-size.png)
