...
category: general
date: 2026-06-17
description: 快速將工作表轉換為 C# DataTable。學習如何在 C# 中將 Excel 檔案讀取至 DataTable，並使用實際程式碼將 Excel
  匯出為 DataTable。
draft: false
keywords:
...
language: zh-hant
og_description: 快速將工作表轉換為 C# DataTable。本教學示範如何將 Excel 檔案讀取至 C# DataTable，以及如何將 Excel
  匯出為 C# DataTable，並提供完整範例。
og_title: 在 C# 中將工作表轉換為 DataTable – 完整指南
schemas:
- author: Aspose
...
- Excel
- DataTable
title: 將工作表轉換為 C# 中的 DataTable – 完整程式設計指南
url: /zh-hant/net/excel-data-import-export/convert-worksheet-to-datatable-in-c-complete-programming-gui/
---
...
| 常見問題 | 為何會發生 | 解決方法 |
|----------|------------|----------|
| **檔案被其他程序鎖定** | 未使用 `LoadOptions` 開啟工作簿會保持檔案句柄開啟。 | 使用 `LoadOptions` 搭配 `MemorySetting.MemoryPreference`，或將 `Workbook` 包在 `using` 區塊中。 |
| **缺少欄位標題** | 若第一列是資料而非標題，`ExportDataTable` 會把它當作資料。 | 為 `includeColumnNames` 參數傳入 `false`，然後自行加入欄位名稱。 |
| **混合資料型別導致例外** | `ExportAsString` 為 `false` 時，數值會變成 `double`，日期會變成 `DateTime`。 | 保持 `ExportAsString = true`，除非需要強型別，否則自行處理轉換。 |
| **巨量工作表導致記憶體不足** | 一次匯出數百萬列會耗盡堆疊。 | 分批匯出：以區塊方式迭代列，然後合併 `DataTable`。 |
