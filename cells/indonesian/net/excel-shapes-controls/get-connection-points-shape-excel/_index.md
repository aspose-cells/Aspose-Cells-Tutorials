...
Hal pertama yang harus kita lakukan adalah membuat sebuah instance dari `Workbook` class. Ini merupakan file Excel di Aspose.Cells. Jika Anda tidak memiliki file, tidak masalah—Anda dapat memulai dengan buku kerja kosong.
```csharp
// Új munkafüzet példányosítása
Workbook workbook = new Workbook();
```
Pada langkah ini, kami telah membuat buku kerja Excel kosong, tetapi Anda juga dapat memuat buku kerja yang sudah ada dengan meneruskan jalur file ke `Workbook` konstruktőr.
...
Setelah kita menambahkan kotak teks, kotak tersebut menjadi bagian dari koleksi bentuk lembar kerja. Sekarang kita akan mengakses bentuk tersebut menggunakan `Shapes` gyűjtemény.
```csharp
...
```
Loop ini mengulangi setiap titik koneksi dan mencetak `X` és `Y` koordinat. Ini dapat berguna untuk men-debug atau mengonfirmasi secara visual titik koneksi suatu bentuk.
## Langkah 7: Jalankan dan Selesaikan
Setelah Anda menyiapkan semua langkah di atas, Anda dapat menjalankan kodenya. Berikut baris terakhir yang memastikan proses selesai dengan sukses:
```csharp
...
Ya, meskipun Anda dapat mencobanya secara gratis, lisensi diperlukan untuk mendapatkan fitur lengkap. Anda dapat [beli lisensi di sini](https://purchase.aspose.com/buy) vagy szerezz egy [ideiglenes engedély](https://purchase.aspose.com/temporary-license/).
### Bagaimana cara menambahkan berbagai jenis bentuk di Aspose.Cells?
Használhatod a `Add` metode untuk bentuk seperti persegi panjang, elips, dan lainnya. Setiap bentuk memiliki parameter khusus yang dapat Anda sesuaikan.
