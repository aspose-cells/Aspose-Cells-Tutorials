...
- Aspose.Cells untuk .NET terinstal. [Töltsd le itt](https://releases.aspose.com/cells/net/).
- Pengetahuan dasar tentang C# dan kerangka kerja .NET.
- Lisensi yang valid untuk Aspose.Cells. Jika Anda belum memilikinya, dapatkan lisensi [ingyenes próba](https://releases.aspose.com/) vagy kérjen egy [ideiglenes engedély](https://purchase.aspose.com/temporary-license/).
- IDE seperti Visual Studio.
- .NET Framework 4.0 atau lebih tinggi.
Jika Anda kehilangan sesuatu, jangan khawatir. Anda selalu dapat mengunjungi kembali tautan ini untuk mengunduh file yang diperlukan atau mencari bantuan dari [Aspose támogatói fórum](https://forum.aspose.com/c/cells/9).
...
```
- Kami menambahkan tanggal sistem saat ini ke sel `A1` használva `PutValue(DateTime.Now)`.
- Kami mengambil gaya sel saat ini `A1` használva `GetStyle()`.
...
```
- A `Save()` metode menyimpan buku kerja dalam direktori yang ditentukan.
...
## GYIK
### Bagaimana cara menerapkan opsi pemformatan yang lebih kompleks di Aspose.Cells?
...
### Dapatkah saya menerapkan format angka khusus ke rentang sel?
Ya, Aspose.Cells memungkinkan Anda menerapkan gaya ke rentang sel menggunakan `Range.SetStyle()` módszer.
### Format file apa lagi yang dapat saya gunakan untuk menyimpan buku kerja?
Aspose.Cells mendukung banyak format, termasuk XLSX, CSV, dan PDF. Cukup ubah `SaveFormat` a `Save()` módszer.
...
Aspose.Cells menawarkan uji coba gratis, tetapi untuk fungsionalitas penuh, Anda memerlukan lisensi yang valid. Anda bisa mendapatkannya [ideiglenes jogosítvány itt](https://purchase.aspose.com/temporary-license/).
