...
- Aspose.Cells untuk .NET: Pastikan Anda telah mengunduh dan menginstalnya. Jika belum, unduh dan instal [itt](https://releases.aspose.com/cells/net/).
...
- Lisensi Sementara atau Penuh: Aspose.Cells memerlukan lisensi. Jika Anda baru mencobanya, dapatkan lisensi [ingyenes ideiglenes engedély](https://purchase.aspose.com/temporary-license/) untuk menghindari keterbatasan evaluasi.
...
A fenti kódban cserélje ki a `"YOUR DOCUMENT DIRECTORY"` dengan jalur sebenarnya tempat file Excel Anda berada. `Workbook` kelas memuat berkas Excel ke dalam memori.
...
A `Split()` metode, Anda memberi tahu Aspose.Cells untuk membagi lembar kerja di sel aktif (`A20` dalam kasus ini). Dari titik ini, Excel membuat pembagian pada lembar yang memisahkan panel agar Anda dapat menavigasi secara independen.
...
Itt a `Save()` metode menyimpan buku kerja dengan panel yang baru Anda bagi menjadi file Excel keluaran. Perubahan yang Anda buat kini siap untuk Anda—atau siapa pun—gunakan.
...
Ya, Anda dapat membagi lembar kerja menjadi beberapa panel dengan menentukan sel aktif yang berbeda dan memanggil `Split()` módszer.
...
Igen, a `Split()` Metode ini berfungsi untuk format XLS dan XLSX.

### Használhatom az Aspose.Cells-t licenc nélkül?  
Ya, tetapi ada batasannya. Untuk pengalaman yang lengkap, sebaiknya gunakan [ideiglenes](https://purchase.aspose.com/tempvagyary-license/) or [lisensi berbayar](https://purchase.aspose.com/buy).
