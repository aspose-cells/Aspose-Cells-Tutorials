...
- **Uji Coba Gratis**Mulailah dengan uji coba gratis 30 hari untuk menguji kemampuan penuhnya.
...
Di sini, kami memuat file Excel (`sourceFile.xlsx`) dan sertifikat PKCS#12 (`pfxFile.pfx`). Itu `password` digunakan untuk membuka kunci sertifikat Anda.
...
Itu `DigitalSignature` objek diinisialisasi dengan KeyStore dan stempel waktu Anda. Metode `setXAdESType` mengonfigurasi tanda tangan untuk mematuhi standar XAdES.
...
Itu `DigitalSignatureCollection` memegang tanda tangan kita, yang kemudian dikaitkan dengan buku kerja menggunakan `setDigitalSignature`.
...
2. **Pelaporan Keuangan**: Tingkatkan kepercayaan dengan menandatangani laporan keuangan.
...
4. **Di mana saya bisa mendapatkan dukungan untuk masalah Aspose.Cells?**
