...
- **Uji Coba Gratis**Dapatkan lisensi uji coba untuk menguji fitur.
...
Di Sini, `Workbook` adalah kelas yang disediakan oleh Aspose.Cells yang mewakili berkas Excel. Dengan menentukan jalur berkas, Anda memuatnya ke dalam aplikasi Java Anda.
...
```
Itu `Cells` Objek mewakili semua data sel dalam lembar kerja, yang memungkinkan Anda melakukan berbagai operasi pada sel individual atau rentang sel.
...
```
Itu `CellArea` menentukan sel awal dan akhir dari suatu rentang. `moveRange` metode menggeser area ini dengan offset baris dan kolom yang ditentukan.
...
```
Itu `save` metode menulis modifikasi Anda kembali ke dalam file Excel, menggunakan format yang ditentukan.
...
4. **Apakah ada dukungan untuk format Excel yang berbeda seperti .xlsx atau .xlsm?**
   - Ya, Aspose.Cells mendukung berbagai format Excel termasuk `.xls`Bahasa Indonesia: `.xlsx`, Dan `.xlsm`.
