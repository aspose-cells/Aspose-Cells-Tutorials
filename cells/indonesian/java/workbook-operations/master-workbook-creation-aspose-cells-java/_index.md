...
1. **Uji Coba Gratis**:Anda dapat mengunduh versi uji coba Aspose.Cells dari [Di Sini](https://releases.aspose.com/cells/java/).
2. **Lisensi Sementara**:Jika Anda perlu mengevaluasi semua fitur tanpa batasan apa pun, dapatkan lisensi sementara [Di Sini](https://purchase.aspose.com/temporary-license/).
...
- **setSubskrip()**: Menerapkan pemformatan subskrip pada teks.

## Aplikasi Praktis
Aspose.Cells untuk Java bersifat serbaguna dan dapat digunakan dalam berbagai skenario:
1. **Pelaporan Keuangan**: Secara otomatis membuat laporan terperinci dengan nilai mata uang yang disesuaikan.
2. **Analisis Data**: Buat dasbor dengan mengisi data dari basis data ke dalam lembar Excel.
3. **Manajemen Inventaris**: Mengelola catatan inventaris dengan unit subskrip yang mudah dibaca.
...
1. **Bagaimana cara mengatur teks superskrip, bukan subskrip?**
   - Menggunakan `font.setSuperscript(true);` menggantikan `setSubscript()`.
...
4. **Bagaimana cara menyimpan buku kerja dalam format yang berbeda?**
   - Menggunakan `workbook.save("path/to/file", SaveFormat.XLSX);` Di mana `SaveFormat` menentukan format yang diinginkan.
