...
Pengguna Gradle harus menambahkan ini ke `build.gradle` mengajukan:
...
Aspose.Cells menawarkan lisensi uji coba gratis, yang dapat Anda peroleh dari [Di Sini](https://purchase.aspose.com/temporary-license/)Ini memungkinkan akses penuh ke semua fitur tanpa batasan untuk waktu terbatas.
...
2. **Konfigurasi Lisensi**:
   - Unduh dan terapkan lisensi sementara menggunakan cuplikan kode ini setelah menginisialisasi `Workbook` obyek:

    ```java
    License license = new License();
    license.setLicense("path/to/your/license/file");
    ```
3. **Inisialisasi Dasar**:Membuat sebuah `Workbook` contoh untuk mulai bekerja dengan file Excel.
...
**Penjelasan**: Di Sini, `ws.getCharts().get(0)` mengambil bagan pertama dari kumpulan bagan di lembar kerja.
...
**Konfigurasi Kunci**: `DataLabelShapeType.WEDGE_ELLIPSE_CALLOUT` mengatur bentuk menjadi gelembung ucapan oval, menambahkan gaya visual.
...
1. **Pelaporan Keuangan**: Gunakan bentuk khusus untuk menyorot metrik keuangan utama.
2. **Konten Edukasi**Meningkatkan grafik pada materi pendidikan untuk keterlibatan yang lebih baik.
...
- **Optimalkan Penggunaan Memori**: Tutup buku kerja segera setelah digunakan untuk mengosongkan memori.
- **Pemrosesan Batch**: Memproses beberapa grafik secara massal jika berlaku.
- **Penanganan Data yang Efisien**: Minimalkan operasi dalam loop untuk kinerja yang lebih baik.
...
Untuk lebih mengeksplorasi kemampuan Aspose.Cells, pertimbangkan untuk bereksperimen dengan opsi penyesuaian lain seperti warna dan font. Untuk pertanyaan atau bantuan apa pun, lihat [Forum dukungan Aspose](https://forum.aspose.com/c/cells/9).

## Bagian FAQ

1. **Bagaimana cara mengubah bentuk beberapa label data?**
   - Ulangi setiap seri di bagan Anda dan terapkan `setShapeType()` sesuai kebutuhan.
...
4. **Apakah ada cara untuk mengembalikan bentuk label data ke default?**
   - Mengatur `DataLabelShapeType.DEFAULT` untuk mengembalikan perubahan.
