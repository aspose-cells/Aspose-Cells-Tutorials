...
language: en
og_description: How to link sheets in Excel using SmartMarkerProcessor. Learn to populate
  master sheet and generate a master detail report in minutes.
og_title: How to Link Sheets in Excel with SmartMarker – Step‑by‑Step
schemas:
- author: Aspose
...
- C#
- master‑detail
title: How to Link Sheets in Excel with SmartMarker – Step‑by‑Step Guide
url: /net/smart-markers-dynamic-data/how-to-link-sheets-in-excel-with-smartmarker-step-by-step-gu/
---
...
# How to Link Sheets in Excel with SmartMarker – Step‑by‑Step Guide

Ever wondered **how to link sheets** in Excel without manually copying rows or writing endless VBA loops? You're not alone. Most developers hit a wall when they need a clean master‑detail report that stays in sync as data changes. The good news? SmartMarkerProcessor does the heavy lifting for you, turning a few lines of C# into a fully‑fledged master‑detail workbook.

In this tutorial we’ll walk through the exact steps to **populate master sheet**, set up the detail sheet, and finally **generate master detail report** that updates automatically. By the end you’ll have a reusable pattern you can drop into any .NET project.
...
**Why this matters:** SmartMarker doesn’t magically guess relationships; it looks for matching property names (`MasterId` → `Id`). By structuring the data this way we give the processor a clear map, which is the cornerstone of **how to link sheets** effectively.
...
**Why enable `MasterDetail`?** Without this flag, the processor would treat `{MasterSheet}#master` and `{DetailSheet}#detail` as independent operations, losing the crucial relationship between rows. Setting the flag is the single line that makes **how to link sheets** actually work.
...
Open the file and you’ll see two sheets: *MasterSheet* lists `A` and `B`, while *DetailSheet* shows `Item1` under master `1` and `Item2` under master `2`. That’s the essence of **populate master sheet** and **generate master detail report** in one go.
