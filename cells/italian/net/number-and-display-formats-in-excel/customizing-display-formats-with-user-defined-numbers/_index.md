---
"description": "Scopri come personalizzare i formati di visualizzazione con Aspose.Cells per .NET. Formatta date, percentuali e valute con questa guida dettagliata."
...
- Una licenza valida per Aspose.Cells. Se non ne hai una, procuratene una [prova gratuita](https://releases.aspose.com/) o richiedi un [licenza temporanea](https://purchase.aspose.com/temporary-license/).
- Un IDE come Visual Studio.
- .NET Framework 4.0 o versione successiva.
Se ti manca qualcosa, non preoccuparti. Puoi sempre rivisitare questi link per scaricare i file necessari o chiedere aiuto al [Forum di supporto di Aspose](https://forum.aspose.com/c/cells/9).
...
Per prima cosa, hai bisogno di un posto dove salvare i tuoi file, giusto? Creiamo una directory in cui salvare il file Excel di output. In questo passaggio, ci assicureremo anche che la directory esista prima di salvare qualsiasi cosa.
```csharp
...
Ora che abbiamo la nostra directory, creiamo una nuova cartella di lavoro Excel e aggiungiamoci un foglio di lavoro.
```csharp
...
```
- Per prima cosa creiamo un nuovo `Workbook` oggetto. Consideralo come un file Excel.
...
## Passaggio 3: aggiunta di una data a una cella e personalizzazione del formato
Ora inseriamo la data corrente in una cella e formattiamola per visualizzarla in modo personalizzato. Invece del formato data predefinito, imposteremo un formato personalizzato come `d-mmm-yy`.
```csharp
// Aggiunta della data di sistema corrente alla cella "A1"
...
Ora lavoriamo con i numeri. Aggiungeremo un valore numerico a un'altra cella, ad esempio `A2`e formattarlo come percentuale.
```csharp
...
Aggiungiamo un altro valore, diciamo alla cella `A3`formattarlo per visualizzarlo come valuta. Per rendere le cose più interessanti, useremo un formato che visualizza i valori positivi come valuta in sterline e i valori negativi in dollari.
```csharp
...
Assolutamente! Puoi usare formati numerici personalizzati per visualizzare i numeri negativi con colori o simboli diversi.
### Aspose.Cells per .NET è gratuito?
Aspose.Cells offre una prova gratuita, ma per la piena funzionalità è necessaria una licenza valida. È possibile ottenere una [licenza temporanea qui](https://purchase.aspose.com/temporary-license/).
